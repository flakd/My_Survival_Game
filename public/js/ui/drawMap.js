  ...
  for (let y = 0; y < map.length; y++) {
    for (let x = 0; x < map[y].length; x++) {
      ...
      if (map[y][x] === 1) {
        ...
        if (g.startCellNumStr === '') startCellNumStr = numCellStr;
        cell.innerHTML =
          "<span id='player_" +
          numCellStr +
          "' class='player' style='display:none; z-index:100'>🚶🏻</span>";
        if (numFishes === 0) {
          cell.innerHTML +=
            "<div id='fish_" + numCellStr + "' class='fish'>🐟</div>";
          numFishes++;
        } else if (numTrees === 0) {
          cell.innerHTML +=
            "<div id='tree_" + numCellStr + "' class='tree'>🌲</div>";
          numTrees++;
        } else if (numMountains === 0) {
          cell.innerHTML +=
            "<div id='mountain_" + numCellStr + "' class='mountain'>🏔️</div>";
          numMountains++;
        }
      }
